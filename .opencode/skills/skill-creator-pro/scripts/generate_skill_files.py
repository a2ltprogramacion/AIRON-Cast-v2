#!/usr/bin/env python3
# generate_skill_files.py — AIRON‑Cast
# Generates or updates skill files from a JSON Blueprint spec.
# Usage: python generate_skill_files.py --spec '<json>' --output ./.agents/skills/
#        python generate_skill_files.py --file blueprint.json --output ./.agents/skills/ --force
...
def find_project_root(start_path: str) -> str:
    """Walk up from start_path until we find AGENTS.md."""
...
def check_manifest(root: str) -> bool:
    """Warns if manifest.json is missing. Does not block execution."""
    manifest_path = os.path.join(root, "manifest.json")
    if not os.path.exists(manifest_path):
        print(
            "[ADVERTENCIA] manifest.json no encontrado en la raíz del proyecto. "
...
def main():
    parser = argparse.ArgumentParser(
        description="AIRON‑Cast — Materializa archivos de skill desde un Blueprint JSON."
    )
    ...
    parser.add_argument(
        "--output",
        default=".",
        help="Directorio destino (ej: ./.agents/skills/)."
    )
    parser.add_argument(
        "--force",
        action="store_true",
