...
name: ui-ux-pro-max
description: |
  Proporciona inteligencia de diseño UI/UX con una base de datos de estilos, paletas de colores, tipografías, pautas UX y ejemplos por stack. 
  Actívala cuando el usuario necesite diseñar páginas web o móviles, elegir paletas de colores, tipografías, estilos, o revisar accesibilidad y buenas prácticas. 
  Incluye los 4 Arquetipos Authority Premium, el Mini-Brief de intake y el blueprint de 7 secciones para landings premium.
---
...
### A) "Modern Clinic" (Clínica Moderna)
...
- **Palette**: Ivory/Marfil + True Black + Soft Gold Accent.
...
1. **Apertura (Hero)**  
   – Strong headline + short subhead + 2 CTAs.  
   – Authority badge (use the Verifiable Proof from the brief).

2. **Problema → Solución (Double column)**  
   – Left: real problem (3 bullets).  
   – Right: solution (3 bullets).  
   – Animation: smooth scroll reveal (no fireworks).

3. **Cómo funciona (Timeline)**  
   – 3 numbered steps (01 / 02 / 03).  
   – Each step with a distinct micro‑animation (subtle).

4. **Evidencia (Dynamic social proof)**  
   – Slow carousel of “results” (numbers or mini case studies).  
   – Alternate “before/after” or “situation/result”.  
   – No generic testimonials; sound plausible.

5. **Servicios / Planes**  
   – If pricing exists: 3 plans, middle one highlighted.  
   – If no pricing: 3 service blocks + CTA.

6. **Preguntas rápidas (FAQ)**  
   – 4 real questions (no filler).  
   – Smooth accordion animation.

7. **Cierre**  
   – Strong CTA + low‑friction microcopy (“takes 30 seconds”, “no commitment”).  
   – Clean footer with links and legal.
...
Available domains: `style`, `color`, `typography`, `chart`, `ux`, `landing`, `product`, `react`, `web`, `prompt`.
...
Available stacks: `html-tailwind`, `react`, `nextjs`, `astro`, `shadcn`.
