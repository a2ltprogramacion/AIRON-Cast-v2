#!/usr/bin/env python3
# journal_report.py — AIRON‑Cast
# Generates a pattern analysis report from journal entries since the last report.
# Usage: python journal_report.py --project-slug <slug> [--since-entry <filename>]
...
def parse_entry_meta(filename: str) -> dict:
    ...
    if not match:
        # ADR pattern
        match_adr = re.match(r'^ADR-(\d+)-(.+)-(\d{8})\.md$', filename)
        if match_adr:
            return {
                "timestamp": match_adr.group(3) + "-000000",
                "type":      "adr",
                "slug":      match_adr.group(2),
                "filename":  filename,
            }
        return {}
    return {
...
def read_entry_fields(entry_path: str) -> dict:
    ...
    try:
        ...
        # For task entries: extract agent_name and skills_used
        agent_match = re.search(r'\*\*Agente:\*\*\s*(.+)', content)
        if agent_match:
            fields["agent_name"] = agent_match.group(1).strip()

        skills_match = re.search(r'\*\*Skills utilizadas:\*\*\s*(.+)', content)
        if skills_match:
            fields["skills_used"] = skills_match.group(1).strip()
...
def load_task_counter(journal_dir: str) -> dict:
    ...
    if not os.path.exists(counter_path):
        return {"total_tasks": 0, "last_report_at": 0, "last_report_file": None}
    try:
...
def save_task_counter_report(journal_dir: str, report_filename: str) -> None:
    counter_path = os.path.join(journal_dir, ".task-counter.json")
    counter = load_task_counter(journal_dir)
...
def generate_report(journal_dir: str, since_entry: str | None = None) -> str:
    ...
    if not os.path.exists(entries_dir):
        print("ERROR: Entries directory does not exist.", file=sys.stderr)
        sys.exit(2)
    ...
    all_files = sorted([
        ...
        if f.endswith(".md") and (re.match(r'^\d{8}-\d{6}_', f) or f.startswith("ADR-"))
    ])

    if not all_files:
        print("ERROR: No journal entries found.", file=sys.stderr)
        sys.exit(1)
    ...
    if since_entry:
        try:
            ...
            print(
                f"WARNING: since-entry '{since_entry}' not found. "
        ...
        if last_report_file:
            try:
                # Entries after the last report's timestamp
    ...
    if not target_files:
        print("ERROR: No new entries since last report.", file=sys.stderr)
        sys.exit(1)
    ...
    batch_num = load_task_counter(journal_dir).get("total_tasks", 0)
    report_filename = f"{now.strftime('%Y%m%d')}_pattern-report_batch-{batch_num}.md"
    report_path = os.path.join(reports_dir, report_filename)

    tasks    = entries_by_type.get("task",    [])
    ...
    # --- Task summary ---
    lines.append("\n---\n")
    lines.append(f"## Tareas del Batch ({len(tasks)})\n")
    if tasks:
        lines.append("**Agentes activos:**")
        agent_count = defaultdict(int)
        for t in tasks:
            agent_count[t.get("agent_name", "desconocido")] += 1
        for agent, count in sorted(agent_count.items(), key=lambda x: -x[1]):
            lines.append(f"- {agent}: {count}")

        lines.append("\n**Tareas completadas:**")
        for t in tasks:
            lines.append(f"- `{t.get('slug', '?')}` ({t.get('timestamp', '?')})")
    else:
        lines.append("_No hay tareas en este batch._")
    ...
    # --- Recommendations ---
    lines.append("\n---\n")
    lines.append("## Recomendaciones para el Ecosistema\n")

    recommendations = []

    high_recurrence = [p for p in problems if p.get("recurrence_risk") == "high"] if problems else []
    if high_recurrence:
        recommendations.append(
            "Revisar los componentes afectados por problemas de alto riesgo de recurrencia "
            "y actualizar sus SKILLs o perfiles con las mitigaciones documentadas."
        )
    ...
    if len(patterns) > 0:
        recommendations.append(
            ...
            "Considerar si alguno debe materializarse como skill del ecosistema."
        )
    ...
    lines.append(f"\n---\n*Reporte generado automáticamente por journal_report.py — AIRON‑Cast*")
    ...
    save_task_counter_report(journal_dir, report_filename)
    return report_path
...
def main():
    parser = argparse.ArgumentParser(
        description="AIRON‑Cast Journal — Generate pattern report from journal entries."
    )
    parser.add_argument(
        "--project-slug",
        required=True,
    ...
    parser.add_argument(
        "--since-entry",
        default=None,
        help="Filename of the entry to start from (exclusive)."
    )
